...
<div class="page-header">
    <div>
        <h1 class="page-title">Buat Perusahaan</h1>
        <p class="page-subtitle">Tambahkan perusahaan baru dengan cepat dan rapi.</p>
    </div>
    <a href="{{ route('admin.perusahaan.index') }}" class="btn-secondary">Kembali</a>
</div>
    ...
    <form method="POST" action="{{ route('admin.perusahaan.store') }}">
        ...
        <div class="form-grid">
            ...
            <div class="form-group full-width">
                ...
                <textarea name="deskripsi" class="form-input" style="min-height:140px;">{{ old('deskripsi') }}</textarea>
            </div>
            <div class="form-group full-width">
                <label class="form-label">Overdue (Hari)</label>
                <input
                    type="number"
                    name="overdue"
                    class="form-input"
                    min="1"
                    value="{{ old('overdue', 28) }}"
                    required>
            </div>
        </div>

        <div class="modal-footer" style="justify-content:flex-start; padding:0; border:none; margin-top:12px;">
            <a href="{{ route('admin.perusahaan.index') }}" class="btn-secondary" style="margin-right:12px;">Batal</a>
