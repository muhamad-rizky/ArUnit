...
@if(session('success'))
    <div style="margin-bottom:20px; padding:16px 20px; border-radius:8px; background-color:#ecfdf5; color:#065f46; border-left:4px solid #10b981; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);">
        <strong>Sukses!</strong> {{ session('success') }}
    </div>
@endif

<div style="margin-bottom:20px; display:flex; flex-wrap:wrap; gap:16px; align-items:center; justify-content:space-between; background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #e2e8f0;">
    <form method="GET" action="{{ route('admin.stocks.index') }}" style="display:flex; gap:12px; align-items:center; flex:1; min-width:260px;">
        <input type="text" name="search" class="search-input" value="{{ old('search', $search ?? '') }}" placeholder="Cari No DO, Nama Mobil, No Rangka...">
        <button type="submit" class="btn-search">Cari</button>
    </form>
    @if(!empty($search))
        <a href="{{ route('admin.stocks.index') }}" style="color:#ef4444; text-decoration:none; font-weight:600; padding: 8px 12px; border-radius: 6px; transition: background 0.2s;" onmouseover="this.style.background='#fee2e2'" onmouseout="this.style.background='transparent'">
            Reset Filter
        </a>
    @endif
</div>
        ...
        <tbody>
            @forelse($items as $i => $item)
                @php 
                    ...
                    $rowClass = '';
                    if($statusLower == 'free') $rowClass = 'row-free';
                    elseif($statusLower == 'matching') $rowClass = 'row-matching';
                    elseif($statusLower == 'sold') $rowClass = 'row-sold';
                @endphp
                <tr class="{{ $rowClass }}">
                    <td style="text-align: center;">{{ $items->firstItem() + $i }}</td>
                    <td>{{ $item->no_do }}</td>
                    <td>{{ $item->tanggal_do ? \Carbon\Carbon::parse($item->tanggal_do)->format('d-M-Y') : '-' }}</td>
                    ...
                    <td style="text-align: right; font-weight: 600;">{{ number_format($item->hpp ?? 0, 0, ',', '.') }}</td>
                    <td>{{ $item->lokasi }}</td>
                    ...
                    <td>{{ $item->tanggal_matching_do ? \Carbon\Carbon::parse($item->tanggal_matching_do)->format('d-M-Y') : '-' }}</td>
                    <td>{{ $item->cabang }}</td>
                    ...
                    <td class="col-action-body">
                        <div style="display: flex; gap: 8px; justify-content: center;">
                            ...
                            <form action="{{ route('admin.stocks.destroy', $item) }}" method="POST" style="margin: 0;">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn-action-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">Hapus</button>
                            </form>
                        </div>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="28" style="text-align:center; padding:30px 16px; color: #64748b; background: #fff;">
                        <div style="font-size: 1.1rem; margin-bottom: 8px;">Tidak ada data stock untuk ditampilkan.</div>
                    </td>
                </tr>
            @endempty
        </tbody>
